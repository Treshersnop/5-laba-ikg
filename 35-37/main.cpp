...
public:
    ...
    bool Init()
    {
        ...
        if (!m_box.LoadMesh("D:/Content/box.obj")) {
			return false;
		}

        if (!m_bsphere.LoadMesh("D:/Content/sphere.obj")) {
			return false;
		}
    ...
    {   
        ...
        DSGeometryPass();//вызывает геометрический проход перед проходом света
    ...
    void DSGeometryPass()
    {
		m_DSGeomPassTech.Enable();//разрешение использовать соответствующую технологию

        m_gbuffer.BindForGeomPass();

        // Только геометрический проход обновляет тест глубины
		glDepthMask(GL_TRUE);//запрет записи в буфер глубины где-либо кроме геометрического прохода

		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);//очистка буфера глубины перед записью

		glEnable(GL_DEPTH_TEST);//отключение смешивания
        //рендерим меш
		Pipeline p;
    ...
    //Центр сферы установлен в позиции источника света
	...
	void DSDirectionalLightPass()//для обработки направленного света
	{
		m_gbuffer.BindForLightPass();
