#include "engine_common_28.h"
#include "util_28.h"
#include "particle_system_28.h"
...
struct Particle //структура частицы
{
    float Type;
    Vector3f Pos;
    Vector3f Vel;
    float LifetimeMillis; //время жизни, когда достигнет конца, то частица будет удалена
};
...
//Главная работа этой функции в вызове 2-х private функций, которые обновляют атрибуты частиц и затем рендерят их
...
{
    //начинаем обновление частиц с включения соответствующего метода и устанавливаем некоторые динамические переменные
    m_updateTechnique.Enable();
    ...
    //устанавливка вершинных атрибутов частиц в вершинный буфер
    ...
    glBeginTransformFeedback(GL_POINTS);//активирует transform feedback
    //проверяет входящий буфер и рисует все вершины, которые были в него записаны в пришлый раз
    if (m_isFirst) {
...
void ParticleSystem::RenderParticles(const Matrix4f& VP, const Vector3f& CameraPos)
{
    ...
    glDrawTransformFeedback(GL_POINTS, m_transformFeedback[m_currTFB]);//отрисовка
